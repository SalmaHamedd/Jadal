...
class SurveyErrorView extends StatelessWidget {
  ...
  const SurveyErrorView({super.key, required this.message, required this.onRetry});
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Text(
        message,
        textAlign: TextAlign.center,
        style: const TextStyle(fontFamily: 'Cairo', fontSize: 15, color: JadalColors.judgesGrey),
      ),
    );
  }
}
